"""AWS Machine Learning Blog RSS 크롤러"""
import re
...
class AWSBlogCrawler(BaseCrawler):
    ...
    source = ArticleSource.AWS_BLOG
    name = "AWS ML Blog"

    def __init__(self):
        self.feed_url = CRAWL_SOURCES["aws_blog"]["feed_url"]
    ...
    async def crawl(self, max_items: int = 20) -> list[Article]:
        ...
        try:
            ...
            if feed.bozo and not feed.entries:
                logger.warning(f"AWS Blog 피드 파싱 실패: {feed.bozo_exception}")
                return articles
            ...
            for entry in feed.entries[:max_items]:
                try:
                    ...
                    logger.warning(f"AWS Blog 항목 파싱 실패: {e}")
                    continue

        except Exception as e:
            logger.error(f"AWS Blog 크롤링 실패: {e}")

        logger.info(f"AWS ML Blog 총 {len(articles)}건 수집")
        return articles
